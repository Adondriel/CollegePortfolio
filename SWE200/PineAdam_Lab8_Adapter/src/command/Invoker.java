...
public class Invoker extends JFrame implements ActionListener {
	...
	private Command mostRecentCommand = new TurnNorthCmd();
	...
	/**
	 * Updated to change the mostRecentCommand.
	 */
	@Override
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == north)
	    {
	       mostRecentCommand = northCommand;
	    }
		else if (event.getSource() == south)
	    {
	        mostRecentCommand = southCommand;
	    }
		else if (event.getSource() == east)
	    {
	        mostRecentCommand = eastCommand;
	    }
		else if (event.getSource() == west)
	    {
	        mostRecentCommand = westCommand;
	    }
		else if (event.getSource() == move)
	    {
	        mostRecentCommand = moveCommand;
	    }
		else if (event.getSource() == drop)
	    {
			mostRecentCommand = dropCommand;
	    }
		else if (event.getSource() == get1)
	    {
			mostRecentCommand = get1Command;
	    }
		else if (event.getSource() == get2)
	    {
			mostRecentCommand = get2Command;
	    }
		else if (event.getSource() == attack)
	    {
			mostRecentCommand = attackCommand;
	    }
		else if (event.getSource() == reload)
	    {
			mostRecentCommand = reloadCommand;
	    }		
	}
	/**
	 * 
	...
	/**
	 * Set's the most recent command.
	 * @param mostRecentCommand
	 */
	public void setMostRecentCommand(Command mostRecentCommand) {
		this.mostRecentCommand = mostRecentCommand;
	}
