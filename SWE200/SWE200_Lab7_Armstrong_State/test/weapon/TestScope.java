...
public class TestScope {
	...
	public void testScopePistol() throws AttachmentException, WeaponException {
		...
		assertEquals(35, scopePistol.getMaxRange()); 
		...
		//damage should be 6
		//with the scope, damage should be 6+(1+((35-30)/35)) =7
		assertEquals(7, scopePistol.fire(30)); 
		timer.timeChanged();
		
		//test at base.maxRange (25)
		//Pistol damage is 2
		//with Scope: 2*(1+(1/5)) = 2.4 = 2
		assertEquals(2, scopePistol.fire(25)); 
		timer.timeChanged();
		
		//test out of base.maxRange, but within scope range (30)
		//Pistol damage at 25 is 2
		//with scope boost = 2+5 = 7
		assertEquals(7, scopePistol.fire(30)); 
		timer.timeChanged();
		
		//test at edge of scope range (35)
		assertEquals(7, scopePistol.fire(35)); 
		timer.timeChanged();
	...
	*/
	...
	public void testScopeScopePistol() throws AttachmentException, WeaponException
	{
		...
		//check that the maxRange has been updated
		assertEquals(45, scopeScopePistol.getMaxRange()); 
		
		//check toString()
		assertEquals("Pistol +Scope +Scope", scopeScopePistol.toString()); 
		
		//check that the damage is amped appropriately
		//fire the ScopePistol once at something in range (30), damage is 7
		//with the extra scope, the damage is: 7*(1+((45-30)/45)) = 9
		assertEquals(9, scopeScopePistol.fire(30)); 
		timer.timeChanged();
		
		//test at base.maxRange (35) (scopePistol's maxRange)
		//ScopePistol damage is 7
		//with Scope: 7*(1+(1/6)) = 8
		assertEquals(8, scopeScopePistol.fire(35)); 
		timer.timeChanged();
		
		//test out of base.maxRange, but within scope range (40)
		//ScopePistol damage at 40 is 7
		//with scope boost = 7+5 = 12
		assertEquals(12, scopeScopePistol.fire(40)); 
		timer.timeChanged();
		
		//test at edge of scope range (45)
		assertEquals(12, scopeScopePistol.fire(45)); 
		timer.timeChanged();
	...
	public void testScopeStabilizerPistol() throws AttachmentException, WeaponException
	{
		...
		//check that the damage is amped appropriately
		//fire the stabilizerPistol once at something in range (15)
		//damage should be 10*((25-15+5)/25)*1.25 = 7
		//with the scope, damage should be 7*(1+((35-15)/35)) =11
		assertEquals(11, scopeStabilizerPistol.fire(15));
		...
		//with Scope: 2*(1+(1/5)) = 2.4 = 2
		assertEquals(2, scopeStabilizerPistol.fire(25)); 
		assertEquals(8, scopeStabilizerPistol.getCurrentAmmo()); 
		timer.timeChanged();
		
		//test out of base.maxRange, but within scope range (30)
		//Pistol damage at 30 is 2
		//with Stabilizer: 2
		//with scope boost = 2+5 = 7
		assertEquals(7, scopeStabilizerPistol.fire(30));
		assertEquals(7, scopeStabilizerPistol.getCurrentAmmo()); 
		timer.timeChanged();
		
		//test at edge of scope range (35)
		assertEquals(7, scopeStabilizerPistol.fire(35)); 
	...
	public void testScopePowerBoosterPistol() throws AttachmentException, WeaponException
	{
		...
		//check that the maxRange has been updated
		assertEquals(35, scopeBoosterPistol.getMaxRange()); 
		
		//check toString()
		assertEquals("Pistol +PowerBooster +Scope", scopeBoosterPistol.toString()); 
		
		//check that the damage is amped appropriately
		//fire the boosterPistol once at something in range (15)
		//damage should be 10*((25-30+5)/35)*(1+10/10) = 12
		//with the scope, damage should be 12*(1+((60-30)/60)) =18
		assertEquals(18, scopeBoosterPistol.fire(15));
		...
		assertEquals(3, scopeBoosterPistol.fire(25)); 
		...
		assertEquals(8, scopeBoosterPistol.fire(30));
		assertEquals(7, scopeBoosterPistol.getCurrentAmmo()); 
		timer.timeChanged();
		
		//test at edge of scope range (30)
		//Pistol damage at 50 is 2
		...
		//with Scope: 2*(1+(1/5)) = 2.4 = 2
		assertEquals(2, scopeBoosterPistol.fire(25)); 
		...
		//with powerbooster & 4 bullets: 2*(1+4/10) = 2
		//with Scope: 2*(1+(1/5)) = 2.4 = 2
		assertEquals(0, scopeBoosterPistol.fire(25)); 
		assertEquals(0, scopeBoosterPistol.getCurrentAmmo()); 
		timer.timeChanged();
	}
}
